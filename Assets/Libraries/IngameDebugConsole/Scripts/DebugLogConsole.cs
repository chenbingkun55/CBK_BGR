...
namespace IngameDebugConsole
{
	public class ConsoleMethodInfo
	{
		...
		public bool isMapping => mappingParameterType != null;
		public readonly Type mappingParameterType;
		public readonly PropertyInfo[] mappingProperties;
		...
		public ConsoleMethodInfo(MethodInfo method, Type[] parameterTypes, object instance, string command, string signature, string[] parameters, Type mappingParameterType, PropertyInfo[] mappingProperties)
		{
			this.method = method;
			this.parameterTypes = parameterTypes;
			this.instance = instance;
			...
			this.signature = signature;
			this.parameters = parameters;
			this.mappingParameterType = mappingParameterType;
	...
	public static class DebugLogConsole
	{
		...
		private static readonly List<PropertyInfo> sharedPropertyBuffer = new List<PropertyInfo>( 16 );
		...
		// mapping command
		public static void AddMappingCommand<T>( string command, string description, Action<T> method ) { AddMappingCommand( command, description, method.Method, method.Target, typeof(T) ); }
		public static void AddMappingCommand<T1, T2>( string command, string description, Func<T1, T2> method ) { AddMappingCommand( command, description, method.Method, method.Target, typeof(T1) ); }
		public static void AddMappingCommand( string command, string description, Delegate method, Type parameterType ) { AddMappingCommand( command, description, method.Method, method.Target, parameterType ); }

		private static void AddMappingCommand(string command, string description, MethodInfo method, object instance, Type parameterType)
		{
			if( string.IsNullOrEmpty( command ) )
			{
				Debug.LogError( "Command name can't be empty!" );
			...
			if (methodParameters == null || methodParameters.Length != 1 || !methodParameters[0].ParameterType.IsAssignableFrom(parameterType))
			{
				Debug.LogError("Command method must have one parameter of type " + parameterType);
				return;
			}
			
			sharedPropertyBuffer.Clear();

			var properties = parameterType.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.SetProperty);
			foreach (var property in properties)
			{
				if (property.GetCustomAttribute<IgnoreDataMemberAttribute>() != null)
					continue;
				
				sharedPropertyBuffer.Add(property);
			}
			
			// sharedPropertyBuffer.Sort(SortByName);

			// Store the parameter types in an array
			Type[] memberTypes = new Type[sharedPropertyBuffer.Count];
			
			for( int i = 0; i < sharedPropertyBuffer.Count; i++ )
			{
				Type memberType = sharedPropertyBuffer[i].PropertyType;
				if (parseFunctions.ContainsKey(memberType) || typeof(Component).IsAssignableFrom(memberType) || memberType.IsEnum || IsSupportedArrayType(memberType))
					memberTypes[i] = memberType;
				else
				{
					Debug.LogError(string.Concat("Property ", sharedPropertyBuffer[i].Name, "'s Type ", memberType, " isn't supported"));
					return;
				}
			}
			...
			else
			{
				...
				for( int i = commandFirstIndex; i <= commandLastIndex; i++ )
				{
					int parameterCountDiff = methods[i].parameterTypes.Length - memberTypes.Length;
					if( parameterCountDiff <= 0 )
					{
						// We are sorting the commands in 2 steps:
						// 1: Sorting by their 'command' names which is handled by FindCommandIndex
						...
						{
							int j = 0;
							while( j < memberTypes.Length && memberTypes[j] == methods[i].parameterTypes[j] )
								j++;

							if( j >= memberTypes.Length )
							{
								commandIndex = i;
								commandLastIndex--;
								methods.RemoveAt( i-- );
			...
			// Create the command
			StringBuilder methodSignature = new StringBuilder( 256 );
			string[] parameterSignatures = new string[memberTypes.Length];
			...
			if( memberTypes.Length > 0 )
			{
				...
				for( int i = 0; i < memberTypes.Length; i++ )
				{
					...
					methodSignature.Append( "[" ).Append( GetTypeReadableName( memberTypes[i] ) ).Append( " " ).Append( sharedPropertyBuffer[i].Name ).Append( "]" );
			...
			methods.Insert( commandIndex, new ConsoleMethodInfo( method, memberTypes, instance, command, methodSignature.ToString(), parameterSignatures, methodParameters[0].ParameterType, sharedPropertyBuffer.ToArray() ) );
		}

		private static int SortByName(MemberInfo x, MemberInfo y)
		{
		...
		public static void ExecuteCommand( string command )
		{
			...
			else if (methodToExecute.isMapping)
			{
				try
				{
					// Create an instance of the parameter type
					object parameter = Activator.CreateInstance(methodToExecute.mappingParameterType);
					// Assign the properties of the parameter type
					for (var i = 0; i < parameters.Length; i++)
					{
						var property = methodToExecute.mappingProperties[i];
						var value = parameters[i];
						property.SetValue(parameter, value);
					}
					// Execute the method associated with the command
					object result = methodToExecute.method.Invoke( methodToExecute.instance, new object[]{parameter} );
