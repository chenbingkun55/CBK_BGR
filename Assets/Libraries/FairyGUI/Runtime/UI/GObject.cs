...
namespace FairyGUI
{
    public class GObject : EventDispatcher
    {
        ...
        virtual public void Setup_BeforeAdd(ByteBuffer buffer, int beginPos)
        {
            ...
            {
                this.data = str;
                this.customData = SerializeUtils.Deserialize(str);
            }
        }
        ...
        public GTweener TweenNumText(int endValue, float duration)
        {
            if (!int.TryParse(text, out var startValue))
                startValue = 0;
            
            return GTween.To(startValue, endValue, duration).SetTarget(this, TweenPropType.NumText);
        }
        ...
        {
            return customData != null && customData.TryGetValue(key, out var value) ? value : null;
        }

        public string GetCustomStringProperty(string key)
        {
            return customData != null && customData.TryGetValue(key, out var value) ? value as string : null;
        }

        public int GetCustomIntProperty(string key, int defaultValue = 0)
        {
            return customData != null && customData.TryGetValue(key, out var value) ? (int)value : defaultValue;
        }
        
        public bool GetCustomBoolProperty(string key, bool defaultValue = false)
        {
            return customData != null && customData.TryGetValue(key, out var value) ? (bool)value : defaultValue;
        }

        protected string GetCustomLocalizedText(string fallbackText = null)
        {
            const string Key = "l10nKey";
            
            if (TranslationHelper.GetTextFunc == null)
                return fallbackText;
            
            var textKey = GetCustomStringProperty(Key);
            if (string.IsNullOrEmpty(textKey))
                return fallbackText;
            
            return TranslationHelper.GetTextFunc(textKey, fallbackText);
        }
